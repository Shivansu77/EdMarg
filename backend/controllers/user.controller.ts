...
exports.signupUser = async (req, res) => {
  ...
  try {
    ...
    const mentorProfile = parseProfileField(mentorProfileRaw, mentorProfileRaw || undefined);
    ...
    try {
      await userService.sendEmailVerificationOtp(user.email);
    } catch (otpErr) {
      console.error('[Signup] Failed to send initial OTP:', otpErr.message);
    }
...
exports.sendEmailVerificationOtp = async (req, res, next) => {
  try {
    ...
    const result = await userService.sendEmailVerificationOtp(email);
...
exports.verifyEmailVerificationOtp = async (req, res, next) => {
  try {
    ...
    const updatedUser = await userService.verifyEmailOtp(email, otp);

    const token = await userService.generateToken(updatedUser._id);
    ...
    return res.status(200).json({
      ...
      data: {
        ...buildAuthResponseData(updatedUser, token),
      },
    });
  } catch (err) {
    next(err);
