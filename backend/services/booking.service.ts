...
class BookingService {
  ...
  async createBooking({ studentId, mentorId, date, startTime, sessionType, notes }) {
    ...
    try {
      ...
      if (err.code === 11000) {
        throw new ValidationError('This time slot was just taken by someone else.');
      }
      throw err;
    }

    // Only create Zoom meeting for auto-confirmed video sessions after successful booking creation
    if (autoConfirm && (!sessionType || sessionType === 'video')) {
      ...
      booking = await bookingRepository.updateStatus(booking._id, 'confirmed', {
        ...zoomData,
        ...(zoomError ? { zoomError } : {}),
      });
    }
  ...
  async completeSession(bookingId, mentorId, mentorNotes, { sessionSummary, actionItems, goalId } = {}) {
    ...
    try {
      ...
      completed = await bookingRepository.updateStatus(bookingId, 'completed', updateData, { session });
      
      await userRepository.incrementMentorSessions(mentorId, { session });
