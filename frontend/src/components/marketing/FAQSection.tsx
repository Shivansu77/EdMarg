...
const faqs = [
  ...
  },
  {
    ...
    answer: "Absolutely! We are always looking for experienced professionals to join our community. You can sign up as a student and then apply from the Careers page in your dashboard."
  }
];
...
const FAQItem = ({ faq, isOpen, toggleOpen }: { faq: any; isOpen: boolean; toggleOpen: () => void }) => {
  return (
    <div className="mb-4">
      ...
      >
        <span className={`text-sm sm:text-base font-medium transition-colors duration-300 ${isOpen ? 'text-emerald-900' : 'text-slate-800'}`}>
          {faq.question}
        </span>
        <div
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="p-6 pt-2 text-slate-600 leading-relaxed text-xs sm:text-sm">
              {faq.answer}
            </div>
          </motion.div>
        )}
    ...
    <section className="relative overflow-hidden py-32 bg-slate-50">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 lg:px-8">
        <div className="text-center mb-20">
          ...
          <h2 className="text-2xl font-bold tracking-tight text-slate-950 sm:text-3xl md:text-4xl mb-6 leading-[1.15]">
        ...
        <div className="mt-20 text-center">
          <div className="inline-flex flex-col items-center gap-6 rounded-[2.5rem] border border-white/60 bg-white/40 backdrop-blur-2xl p-10 shadow-xl shadow-slate-200/50 ring-1 ring-black/[0.03]">
            ...
            <button className="inline-flex h-12 items-center justify-center gap-2 rounded-xl bg-slate-950 px-8 text-sm font-semibold text-white shadow-lg shadow-slate-950/20 transition-all hover:bg-slate-800 hover:-translate-y-0.5 active:scale-95">
              Contact Support
            </button>
          </div>
        </div>
